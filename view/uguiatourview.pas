...
begin
  ...
  HTMLFinal := StringReplace(HTMLFinal, '{{PAGE_TITLE}}', Perfil.Titulo + ' | ' + Perfil.Subtitulo, [rfReplaceAll]);
  ...
  // Estrutura visual — "ARTISTA_*" virou "COMERCIO_*", e entrou a categoria
  HTMLFinal := StringReplace(HTMLFinal, '{{COMERCIO_SLUG}}', Perfil.slug, [rfReplaceAll]);
  HTMLFinal := StringReplace(HTMLFinal, '{{COMERCIO_NOME}}', Perfil.Titulo, [rfReplaceAll]);
  ...
  // REMOVIDO: seção "Cuidados Pós-Tattoo" inteira — específica de tatuador,
  // sem equivalente conceitual em pousada/restaurante/passeio. O
  // TComercioPerfil nem tem mais o campo CPosTattoo (removido do model).
  // Se o template index.html do guiatour ainda tiver o bloco
  // {{SECAO_CUIDADOS}}, ele é removido pela limpeza de placeholders não
  // substituídos abaixo — mas o ideal é já tirar esse trecho do próprio
  // HTML do template, pra não carregar marcação morta.
